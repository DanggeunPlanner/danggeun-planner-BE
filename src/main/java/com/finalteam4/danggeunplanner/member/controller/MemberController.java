...
public class MemberController {
    ...
    public ResponseEntity<?> logIn(@RequestBody MemberLogInRequest request, HttpServletResponse response){
        ...
        Boolean isExistUsername = memberService.logIn(request, response);

        return new ResponseEntity<>(new ResponseMessage("로그인 성공", new MemberLogInResponse(isExistUsername)), HttpStatus.OK);
    }
}
